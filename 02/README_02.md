...
つまり、**右下に行くほど X, Y の数値が大きくなる**ということだ。
...
あなたが入力した座標と直径は、**プログラムを実行した瞬間に完全に固定され**、
途中で変化することがないからだ。
...
コンピュータのディスプレイ上で物体が動いているように見せる方法は、
実は今も昔も一つしかない。それはアニメーションだ。

<figure>
<iframe width="560" height="315" src="https://www.youtube.com/embed/fG6ywByMAyE?si=cB7pd224twyqMelB" title="YouTube video player" frameborder="0" disablekb="1" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
<figcaption><small>ゾエトロープ (Zoetrope)</small></figcaption>
</figure>

**少しずつ変化する静止画**を連続的に切り替えて表示することで、
本来動かないはずの絵が**動いているかのような錯覚**を生み出す、という手法のことだ。

---

先ほどの円が動いているかのようにアニメーションさせたいのであれば、
**時間の流れと共に、円の X, Y 座標を少しずつ変えてやれば良い**、ということになる。

しかし、これを実現するには次に説明する道具が必要になる。
...
- ラベルに好きな **名前** を書いて呼ぶことができる。
- 名前を後から変えることはできない。

変数も大体このような性質のものだ。

![bg contain left:30%](jar.png)
...
```js
let 変数名 = 初期値;
var 変数名 = 初期値;  // let ではなく var でも可。
```

`変数名` は容れ物の名前。
`初期値` は容れ物に最初に入れる値。

宣言を行うことで、**その変数が CPU のメモリ上に作成される**。
...
「**100** という値の入った **X** という名前の変数」が作られることになる。

そして以後、この変数: X は **100 という数値と全く同じものとして扱うことが可能**だ。

---


```js
circle(100, 150, 50); // 座標 (100, 150) に直径 50px の円
```

つまり ↑ の命令は、↓ のように置き換えることが可能となる。

```js
circle(X, 150, 50); // 座標 (100, 150) に直径 50px の円
```
...
### 演習:
`variables.js` の変数にいろいろと変更を加え、円や文字列を動かしてみよう。
...
実はこれまでに紹介した *`alert` 命令や `circle` 命令も、関数に他ならない*。
...
関数を呼び出す際、*引数も一緒に渡す* と、
**渡した引数の値によって仕事の結果に変化をもたらす**ことができる。

`alert` 関数なら、引数として渡した文字列が
...
`circle` 関数なら、引数として渡した3つの数値によって
描画される円の位置とサイズが決定される。

---

## p5.js の関数を色々と試してみよう
p5.js には `circle` や `text` 以外にも様々な関数が存在する。
