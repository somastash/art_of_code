...
*データ型*とは、数値や文字列といった値のタイプ（種類）のことだ。
...
*値の後に `,（カンマ）`* を付けることを忘れてはならない。
*最後のプロパティ*の値に限り `,` を付けて付けなくてもどちらでも良いのだが、
「**値の後は必ず `,` を付ける**」と覚えた方が付け忘れが起きにくいだろう。

---

例として一個の :apple: 林檎をオブジェクトとして表してみよう。
...
「*オブジェクト*」と「*プロパティ*」の関係性は、ゲームにおける
「**キャラクター**」と「**ステータス値**」の関係性に近い。
...
このように、**各情報をオブジェクトのプロパティとしてまとめる**ことができる。
...
先ほどの円オブジェクトを `circle()` 関数で描画するなら、以下のように書き表わせる。
...
また、*最後の値に限り* `,` を付けても付けなくてもどちらでも良い。
「**値の後は必ず `,` が付く**」と覚えておけば付け忘れは起きにくいだろう。
...
**`for` ループ**と組み合わせた時にその真価を発揮する。

<hr>

まずはサンプルコードを確認してほしい。
`05/array.js` https://editor.p5js.org/amekusa/sketches/LmP6WozgJ

---

始めに `keylist` という名の変数に、配列を作成・代入している。
...
この配列は `a`, `b`, `c`, 3 つの文字列をそれぞれ要素に持つ。
要素が 3 つなので、**`keylist.length` プロパティの値は `3`** となる。

`draw()` 関数内に要素数を表示している箇所があるので確認してほしい。
...
*ループ回数*を指定する部分に `keylist.length` が入っているので、
この `for` 文は **`keylist` 配列の要素数分だけ実行される**ということになる。
...
そしてループカウンターである*変数 `i`* は、`0` から始まり `要素数 - 1` まで増加するので、
そのまま配列の**添字**として利用しているのが以下の行だ。

```js
  text(keylist[i], x, y); // 要素を表示
```

*`keylist[i]`* とすることで、`keylist` 配列内の**全ての要素**にアクセスしている、
というのが重要な点だ。

---

この方法は、配列内の*最初（0 番目）の要素*から、
*最後（要素数 - 1 番目）の要素*まで順番に精査していくことから、
**配列の走査（スキャン）** と呼ばれることがある。

<hr>

配列走査は非常に*出現頻度が高く*重要な構文なので、
是非とも覚えて使いこなせるようになってほしい。
<small>（プログラムコードに配列が現れたら、ほぼ確実に配列走査も行われると思ったほうがよい。）</small>
